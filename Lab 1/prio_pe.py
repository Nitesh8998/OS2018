...
class process(object):
	...
	def disp(self):
		print "%d  %d  %d   %d   %d  %d  %d"%(self.pid,self.at,self.bt,self.pr,self.ct,self.tat,self.wat)
	...
	while(1):
		...
		if(q.empty()==False):
			...
			if(p.bt==0):
				...
				# if the process comes out only then we see the inserrtion of the
				# if(x<n):
				# 	print "going to push %d "%(x)
